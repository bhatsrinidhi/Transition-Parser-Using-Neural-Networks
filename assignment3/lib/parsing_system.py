...
class ParsingSystem:
    ...
    def apply(self, configuration: Configuration, transition: str) -> Configuration:
        ...
        # TODO(Students) End
        #Noted : configuration contains the dependenct tree structure
        #Noted : transition --> oracle query, with L,R,S entries with the node.
        
        #Since this is Stack structure, 2nd word is last to go in, and first to go out
        #hence word2 will be in position 0, and word1 will be in position1
        ...
        if transition.startswith('L'):
            configuration.add_arc(word2,word1,transition[2:-1])

            #after adding the arc, pop the element from the stack
            configuration.remove_second_top_stack()
        
        if transition.startswith('R'):
            configuration.add_arc(word1,word2,transition[2:-1])

            #after adding the arc to the first element, remove this element from the stack
            configuration.remove_top_stack()
        
        #If neither L or R, then it should be shifted from stack to the buffer. 
        else:
            configuration.shift()

        """
        #as per the paper, for Right arc --wi->wj
        if transition.startswith('R'):
